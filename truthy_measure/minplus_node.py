...
def minplusnodefull(G, retpath=False):
    rows = []
    paths = []
    N = G.shape[0]
    for s in xrange(N):
        row, p = minplusnode(G, s, retpath=retpath)
        rows.append(row)
        paths.append(p)
    return np.asarray(rows), paths

def minplusnode(G, s, targets=None, retpath=False):
    '''
    ...
    paths = []
    for t in targets:
        c, p = minplusnodest(G, s, t, retpath=retpath)
        paths.append(p)
        capacities.append(c)
    capacities = np.asarray(capacities)
    return capacities, paths

def minplusnodest(G, s, t, retpath=False):
    """
            ...
            if len(nbrs)>0:
                for nbr in nbrs:
                    if nbr not in connected:
                        ...
                        if nbr==t:
                            items[nbr][0] = min([dist_nbr, dist_n])
                            if dist_n < dist_nbr:
                                items[nbr][2] = n
                        else:
                            if dist_n + G[n,nbr] < dist_nbr:
                                items[nbr][0] = (dist_n + G[n,nbr])
                                items[nbr][2] = n
                heapify(Q)
            connected.add(n)
